...
// Muestra todas las escuelas con su estado de actividad
...
export default function DashboardAdmin() {
  ...
  useEffect(() => {
    async function cargarDatos() {
      ...
      // Cargar todas las escuelas con su última actualización
      ...
      // Procesar datos — última actualización y problemas abiertos
  ...
  const escuelasFiltradas = escuelas.filter(escuela => {
    const coincideBusqueda = escuela.nombre.toLowerCase().includes(busqueda.toLowerCase()) ||
  ...
  const stats = {
    ...
    alDia: escuelas.filter(e => e.diasSinActualizar !== null && e.diasSinActualizar <= 14).length,
    sinActualizar: escuelas.filter(e => !e.ultimaActualizacion || e.diasSinActualizar > 14).length,
    conProblemas: escuelas.filter(e => e.problemasAbiertos > 0).length,
  }

  function getEstadoBadge(escuela: any) {
    if (!escuela.ultimaActualizacion || escuela.diasSinActualizar > 14) {
      return <span className="badge-regular">Sin actualizar</span>
    }
    if (escuela.problemasAbiertos > 0) {
      return <span className="badge-problema">Con problemas</span>
    }
    if (escuela.ultimaActualizacion.estado === 'bien') {
      return <span className="badge-bien">Al día</span>
    }
    if (escuela.ultimaActualizacion.estado === 'regular') {
      return <span className="badge-regular">Regular</span>
    }
    return <span className="badge-mal">Mal</span>
  }
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Al día', valor: stats.alDia, icon: '✅', color: 'bg-primary-50' },
                    ...
                    <div>
                      ...
                      <p className="text-xs text-neutral-400">
                        ...
                        {escuela.diasSinActualizar !== null && (
                          <span> · Última actualización hace {escuela.diasSinActualizar} días</span>
                        )}
                        {escuela.diasSinActualizar === null && (
                          <span> · Sin actualizaciones aún</span>
                        )}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {escuela.problemasAbiertos > 0 && (
                      <span className="text-xs text-orange-600 font-medium">
                        {escuela.problemasAbiertos} problema{escuela.problemasAbiertos > 1 ? 's' : ''}
                      </span>
                    )}
                    {getEstadoBadge(escuela)}
                  </div>
                </Link>
              ))}
            </div>
